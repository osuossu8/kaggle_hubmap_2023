...
class DataSplitter(object):
    ...
    def split_kfold(df: pd.DataFrame, cfg: BaseConfig) -> pd.DataFrame:
        ...
        for n, (trn_index, val_index) in enumerate(
            kf.split(X=df, y=df[cfg.target_col])
        ):
    ...
    def split_group(df: pd.DataFrame, cfg: BaseConfig) -> pd.DataFrame:
        ...
        for n, (trn_index, val_index) in enumerate(
            kf.split(X=df, groups=df[cfg.group_col])
        ):
            df.loc[val_index, "kfold"] = int(n)
        df["kfold"] = df["kfold"].astype(int)
        return df
    ...
    def split_stratified_group(df: pd.DataFrame, cfg: BaseConfig) -> pd.DataFrame:
        ...
        kf = StratifiedGroupKFold(
            n_splits=cfg.num_fold, shuffle=True, random_state=cfg.seed
        )
        for n, (trn_index, val_index) in enumerate(
            kf.split(X=df, y=df[cfg.target_col], groups=df[cfg.group_col])
        ):
            df.loc[val_index, "kfold"] = int(n)
        df["kfold"] = df["kfold"].astype(int)
        return df
